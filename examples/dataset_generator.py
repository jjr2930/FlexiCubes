...
if __name__ == "__main__":
    ...
    total_views = azimuth_steps * elevation_steps
    print(f"Generating {total_views} views using multithreading...")
    
    # 렌더링 작업을 처리하는 함수
    def render_single_view(params):
        i, j, azimuth, elevation = params
        ...
        # 파일 저장 (RGB 채널만 사용)
        view_filename = f"view_{i:03d}_{j:03d}.png"
        mask_filename = f"mask_{i:03d}_{j:03d}.png"
        view_path = os.path.join(FLAGS.out_dir, view_filename)
        mask_path = os.path.join(FLAGS.out_dir, mask_filename)
        ...
        # 데이터셋 항목 생성
        dataset_item = dict()
        dataset_item["view_path"] = view_filename
        dataset_item["mask_path"] = mask_filename
    ...
    for i in range(azimuth_steps):
        ...
        for j in range(elevation_steps):
            elevation = -80 + j * elevation_delta
            tasks.append((i, j, azimuth, elevation))

    # 멀티스레드로 렌더링 실행 (CUDA 연산이 있으므로 워커 수를 제한)
    start_time = time.time()
    completed = 0
    
    # CUDA는 멀티스레드에서 문제가 될 수 있으므로 순차 실행 또는 워커 수를 1로 제한
    # 실제로는 I/O 작업만 멀티스레드로 처리하는 것이 안전함
    with ThreadPoolExecutor(max_workers=1) as executor:
        futures = {executor.submit(render_single_view, task): task for task in tasks}
        
        for future in as_completed(futures):
            i, j, dataset_item = future.result()
            dataset["data"].append(dataset_item)
            completed += 1
            
            if completed % 10 == 0 or completed == total_views:
                elapsed = time.time() - start_time
                progress = (completed / total_views) * 100
                print(f"Progress: {completed}/{total_views} ({progress:.1f}%) - Elapsed: {elapsed:.1f}s")
    
    end_time = time.time()
    print(f"Generated {total_views} views in {end_time - start_time:.2f} seconds.")
